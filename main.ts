...
for (let i = 0; i < names_message.length; i++) {
    const element = names_message[i];
...
let favorite_transportation = ['Civic car','Honda motorcycle'];
for (let i = 0; i < favorite_transportation.length; i++) {
    const element = favorite_transportation[i];
...
let guest_list = ['Furqan','Talha','Farhat'];
for (let i = 0; i < guest_list.length; i++) {
    const element = guest_list[i];
...
console.log(num == 10 || num != 10)
let arr = [2,true,"Talha"]
console.log("Test whether an item is in a ar
